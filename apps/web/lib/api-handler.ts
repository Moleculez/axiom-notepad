...
import { paperThreadsApi } from "@axiom/shared/paper-threads-api";
import { pdfOcrApi } from "@axiom/shared/pdf-ocr-api";
...
async function handleRequest(
  ...
  try {
    ...
    const threadResponse = await paperThreadsApi(request, path, user.id);
    if (threadResponse) return threadResponse;
    const ocrResponse = await pdfOcrApi(request, path, user.id);
